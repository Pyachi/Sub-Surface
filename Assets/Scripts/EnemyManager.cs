...
public static class EnemyManager
{
    ...
    public static void Update()
    {
        ...
        while (true) //pick a random point off the screen and spawn an enemy, if the enemy is out of bounds try again.
        {
            ...
            spawnPosition = _sub.transform.position + new Vector3((float)(Math.Sin(locAngle) * 15), (float)(Math.Cos(locAngle) * 15), 0F);
        ...
        ObjectManager.AddObject(Object.Instantiate(selection, spawnPosition, Quaternion.identity));  //instantiate the enemy and add it to object list
    }
}